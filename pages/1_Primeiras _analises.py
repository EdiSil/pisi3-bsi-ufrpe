...
class CarAnalysisApp:
    ...
    def show_boxplot_by_quilometragem(self):
        ...
        if self.df is not None:
            ...
            fig.update_layout(showlegend=False, yaxis_title="QUILOMETRAGEM", xaxis_title="MARCA")
            st.plotly_chart(fig)
    ...
    def show_histogram_by_brand(self):
        ...
        if self.df is not None:
            ...
            fig = px.bar(vehicle_counts, x='marca', y='unidades', title='HISTOGRAMA DA QUANTIDADE DE VEÍCULOS POR MARCA', 
                         color='marca', color_discrete_map=self.brand_colors)
            fig.update_layout(showlegend=False, yaxis_title="UNIDADES", xaxis_title="MARCA")
            st.plotly_chart(fig)
    ...
    def show_bar_chart_preco_ano(self):
        ...
        if self.df is not None:
            ...
            fig.update_layout(yaxis_title="PREÇO TOTAL (R$)", xaxis_title="ANO", hovermode="x unified", showlegend=False)
            st.plotly_chart(fig)
