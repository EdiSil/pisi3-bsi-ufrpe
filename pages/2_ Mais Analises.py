...
class CarAnalysisApp:
    ...
    def show_boxplot_by_quilometragem(self):
        ...
        fig = px.box(self.df, x='marca', y='quilometragem', title='Boxplot das Marcas por Quilometragem', 
                     color='marca', color_discrete_map=self.brand_colors, hover_data={'marca': True, 'quilometragem': True})
    ...
    def show_histogram_by_brand(self):
        ...
        fig.update_layout(showlegend=False, yaxis_title="Unidades")  # Remove a legenda
        st.plotly_chart(fig)

    def show_bar_chart_preco_ano(self):
        """Exibe um gráfico de barras relacionando preço acumulado por ano."""
        st.subheader("Gráfico de Barras: Relação entre Preços Acumulados por Ano")
        ...
        fig = px.bar(avg_price_per_year, x='ano', y='preco', title='Relação entre Preços Acumulados por Ano', 
    ...
    def run_app(self):
        """Executa o painel interativo para seleção de gráficos."""
        ...
        options = {
            "Boxplot: Quilometragem por Marca": self.show_boxplot_by_quilometragem,
            "Histograma: Quantidade de Veículos por Marca": self.show_histogram_by_brand,
            "Gráfico de Barras: Preços Acumulados por Ano": self.show_bar_chart_preco_ano,
            "Gráfico de Dispersão: Preço x Quilometragem": self.show_scatter_plot
        }

        selected_option = st.sidebar.radio("Selecione um gráfico para exibir:", list(options.keys()))
        if selected_option:
